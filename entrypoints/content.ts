...
class BackgroundMessageHandler {
	...
	startElementSelector(): Promise<string | null> {
		...
		const future = new Promise<string | null>((resolve) => {
			...
			let finished = false;
			const nodeRoots = getAllNodeRoots();
			LOGGER.debug("nodeRoots", nodeRoots);
			...
			const handleClick = (event: MouseEvent) => {
				...
				const target = previousElements[previousElements.length -1];
				previousElements.push(event.target as HTMLElement);
				revertStyles();
				
				// Generate CSS selector for the element
				const selector = this.generateSelector(target  ?? event.target as HTMLElement);
			...
			const cleanup = () => {
				for (const nodeRoot of nodeRoots) {
					nodeRoot.removeEventListener('mouseover', handleMouseOver, true);
					nodeRoot.removeEventListener('click', handleClick, true);
					nodeRoot.removeEventListener('keydown', handleEscape, true);
				}
			...
			for (const nodeRoot of nodeRoots) {
				nodeRoot.addEventListener('mouseover', handleMouseOver, true);
				nodeRoot.addEventListener('click', handleClick, true);
				nodeRoot.addEventListener('keydown', handleEscape, true);
			}
	...
	generateSelector(element: HTMLElement): string {
		...
		while (el && el.nodeType === Node.ELEMENT_NODE) {
			...
			// Do not use a id that ends with 3 numeric chars. We expect it to be autogenerated and that it change on every render
			if (el.id && is_numeric_char(el.id, el.id.length -1) && is_numeric_char(el.id, el.id.length -2) && is_numeric_char(el.id, el.id.length -3)) {
				part += "#"+el.id;
				parts.unshift(part);
				break;
			}

			// Most formcontrol have the name attribute
...
export default defineContentScript({
	...
	async main() {
		...
		const shared = await get_shared_data(LOGGER, COMMANDER, 10000);
